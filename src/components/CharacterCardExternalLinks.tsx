...
import mapleScouterLogo from '../assets/MapleScouterLogo.svg'
...
function CharacterCardExternalLinks(props: CharacterCardExternalLinksProps) {
    ...
    const logos = useMemo(() => {
        if (currentServer === MapleServer.KMS) {
            return (
                <>
                    <img className="character-card-logo-image character-card-logo" src={mapleScouterLogo} style={{width: 14, height: 14}} alt="MapleScouter" onClick={() => window.open(mapleScouterURL, '_blank')}/>
                </>
            )
        ...
        } else if (currentServer === MapleServer.TMS) {
            return (
                <>
                    <img className="character-card-logo-image character-card-logo" src={mapleScouterLogo} style={{width: 14, height: 14}} alt="MapleScouter" onClick={() => window.open(mapleScouterURL, '_blank')}/>
                </>
